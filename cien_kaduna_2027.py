...
def _gcslc_arm_live_checkout_handshake(*, mode: str = "card") -> str:
    ...
    if not (
        ...
        return ""
    _payer = str(st.session_state.get("cien_executive_payer_name") or "").strip() or CIEN_DEFAULT_EXECUTIVE_PAYER_NAME
    ok, _msg, data = _gcslc_init_live_checkout("chairman@gcslc.ng", mode=mode, payer_name=_payer)
    if not ok or not data.get("authorization_url"):
        st.session_state["cien_checkout_init_failed"] = True
        return ""
    url = str(data["authorization_url"]).strip()
    st.session_state["cien_checkout_ready_url"] = url
    st.session_state["cien_checkout_ready_ref"] = str(data.get("reference", "") or "")
    _prov = str(data.get("_cien_provider") or "").strip().lower()
    if _prov not in ("paystack", "flutterwave"):
        _prov = "paystack" if _gcslc_effective_paystack_secret() else "flutterwave"
    st.session_state["cien_checkout_provider"] = _prov
    st.session_state["cien_checkout_channel"] = mode
    return url
...
def _render_sidebar_live_payment_gateway() -> None:
    ...
    with st.expander("Executive mandate · Emergency trust (no gateway)", expanded=False):
        st.markdown(
            ...
            "<p>Use only when the live payment page cannot be reached — "
            "this path arms the lattice immediately.</p>"
            "</div>",
            unsafe_allow_html=True,
        )
...
def _sidebar_sovereign_social_strip_html() -> str:
    """X, LinkedIn, TikTok, Facebook, Instagram — set ``GCSLC_SOCIAL_*_URL`` in ``st.secrets`` to override defaults."""

    def href(*keys: str, default: str) -> str:
        for k in keys:
            v = _gcslc_streamlit_secrets_get(k)
            if v is not None:
                s = str(v).strip()
    ...
    x_u = href("GCSLC_SOCIAL_X_URL", "SOCIAL_X_URL", default="https://x.com/")
    li_u = href("GCSLC_SOCIAL_LINKEDIN_URL", "SOCIAL_LINKEDIN_URL", default="https://www.linkedin.com/")
    tt_u = href("GCSLC_SOCIAL_TIKTOK_URL", "SOCIAL_TIKTOK_URL", default="https://www.tiktok.com/")
    fb_u = href("GCSLC_SOCIAL_FACEBOOK_URL", "SOCIAL_FACEBOOK_URL", default="https://www.facebook.com/")
    ig_u = href("GCSLC_SOCIAL_INSTAGRAM_URL", "SOCIAL_INSTAGRAM_URL", default="https://www.instagram.com/")
    return (
        '<div class="wa-gateway-wrap"><div class="wa-gateway-inner">'
        "<h4>SOVEREIGN SOCIAL SURFACE</h4>"
        "<p class=\"wa-gateway-sub\">X · LinkedIn · TikTok · Facebook · Instagram — lattice outbound</p>"
        '<div class="wa-gateway-btn-stack">'
        f'<a class="wa-sidebar-wa-link" href="{x_u}" target="_blank" rel="noopener noreferrer">'
        "X (Twitter) · Sovereign broadcast</a>"
        f'<a class="wa-sidebar-wa-link" href="{li_u}" target="_blank" rel="noopener noreferrer">'
        "LinkedIn · Executive lane</a>"
        f'<a class="wa-sidebar-wa-link" href="{tt_u}" target="_blank" rel="noopener noreferrer">'
        "TikTok · Grassroots pulse</a>"
        f'<a class="wa-sidebar-wa-link" href="{fb_u}" target="_blank" rel="noopener noreferrer">'
        "Facebook · Community grid</a>"
        f'<a class="wa-sidebar-wa-link" href="{ig_u}" target="_blank" rel="noopener noreferrer">'
        "Instagram · Visual strike</a>"
        "</div></div></div>"
    )
...
# UPG-only: Goldman + pulsing navy/gold prism frame (sidebar matte chrome stays in _CSS).
_ELECTRIFIED_SOVEREIGN_SIDEBAR_CSS = """
<style>
@import url('https://fonts.googleapis.com/css2?family=Goldman:wght@400;700&display=swap');
.upg-electrified-prism {
  font-family: 'Goldman', cursive, sans-serif;
  border: 2px solid #d4af37;
  padding: 12px 14px 14px 14px;
  border-radius: 12px;
  background: linear-gradient(145deg, #001b3a, #000c1d);
  box-shadow: 0 0 15px rgba(212, 175, 55, 0.4);
  text-align: center;
  animation: upg-prism-sovereign-glow 3s infinite alternate;
  margin: 0.35rem 0 0.5rem 0;
}
@keyframes upg-prism-sovereign-glow {
  from { box-shadow: 0 0 10px rgba(212, 175, 55, 0.3); }
  to { box-shadow: 0 0 25px rgba(212, 175, 55, 0.8); }
}
.upg-electrified-label { color: #d4af37; font-weight: bold; letter-spacing: 1px; margin-bottom: 8px; font-size: 0.72rem; }
.upg-electrified-sub { color: #d4af37; font-size: 0.68rem; margin: 0; opacity: 0.92; line-height: 1.35; }
</style>
"""
...
{log}
    ...
    with st.sidebar:
        ...
        st.markdown(_sidebar_sovereign_social_strip_html(), unsafe_allow_html=True)
        _render_sovereign_ingestion_monitor_sidebar()
        st.markdown(
