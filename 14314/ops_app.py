...
st.markdown(
    ...
    <style>
      ...
      /* Video 1 benchmark — smooth S24 glide (static layer; avoids fragment re-injection flicker) */
      .ops-scroll-track { display: block; animation: opsScrollWard 52s linear infinite; }
...
_PLOTLY_MOBILE = {"displayModeBar": False, "responsive": True}
...
def national_zone_color_scale() -> list:
    """Cached static national layer colors (anti-flicker; stable across delta ticks)."""
    return [[0, "#8B0000"], [0.5, "#00CED1"], [1, "#D4AF37"]]


@st.cache_data(ttl=3600, show_spinner=False)
def national_status_color_map() -> dict:
...
def ops_mirror_delta_panel() -> None:
    ...
    monitor_df["node_status"] = monitor_df.apply(
        lambda r: "Idle Alert"
        if int(r["conversions_7d"]) <= 0
        else (
            "Verified"
            if float(r["canvasser_diligence_score"]) >= 76.0
            else ("Active" if float(r["canvasser_diligence_score"]) >= 52.0 else "Idle")
        ),
        axis=1,
    )
    ...
    with left:
        fig_zone = px.bar(
            ...
            y="ward_clusters",
            color="diligence",
            color_continuous_scale=national_zone_color_scale(),
            title="National Map · Ward-Level Cluster Load by Zone",
        )
        fig_zone.update_layout(
            uirevision="national-zone-v1",
            template=None,
        ...
        st.plotly_chart(fig_zone, use_container_width=True, config=_PLOTLY_MOBILE, key="ops_zone_bar")
    ...
    status_df = monitor_df.groupby("node_status", as_index=False).agg(
        ward_clusters=("ward_clusters", "sum"), assigned_nodes=("assigned_nodes", "sum")
    )
    ...
    fig_status = px.bar(
        ...
        y="assigned_nodes",
        color="node_status",
        color_discrete_map=national_status_color_map(),
        text="assigned_nodes",
        title="Node Activation State (Color-Coded)",
    )
    fig_status.update_layout(
        uirevision="national-status-v1",
        template=None,
        paper_bgcolor="rgba(0,0,0,0)",
    ...
    st.plotly_chart(fig_status, use_container_width=True, config=_PLOTLY_MOBILE, key="ops_status_bar")
