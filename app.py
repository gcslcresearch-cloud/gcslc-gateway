...
_14314_DIR = os.path.join(_ROOT_DIR, "14314")
if not os.path.isfile(os.path.join(_BUNDLE_DIR, "data_engine.py")) and os.path.isfile(
    os.path.join(_14314_DIR, "data_engine.py")
):
    _BUNDLE_DIR = _14314_DIR
if _BUNDLE_DIR not in sys.path:
    sys.path.insert(0, _BUNDLE_DIR)

# Prompt 204 — Convener: strict port 8505 (see repo root `.streamlit/config.toml`).
...
def _resolve_config_path(filename: str) -> str:
    for d in (_ROOT_DIR, _BUNDLE_DIR):
        p = os.path.join(d, filename)
        if os.path.isfile(p):
            return p
    return os.path.join(_ROOT_DIR, filename)


_LEADERSHIP_JSON = _resolve_config_path("leadership.json")
_EXEC_SYNC_RECIPIENTS_JSON = _resolve_config_path("executive_sync_recipients.json")
...
_wm_cells = "".join('<span class="rhgi-wm-cell">RHGI</span>' for _ in range(48))
st.markdown(
...
st.markdown(
    ...
    <div class="rhgi-brand-block">
      ...
      <p class="rhgi-mandate-gold">Securing the 20.7 Million Votes Mandate</p>
      <p class="rhgi-mandate-gold">15/15 Sovereign Mirror</p>
...
st.markdown(
    '<p class="rhgi-signature">Renewed Hope Grassroots Initiatives (RHGI) — Office of the DG.</p>',
    unsafe_allow_html=True,
)
...
with tab_global:
    ...
    st.caption(
        "Grassroots context: food inflation vs growth — read alongside the 20.7M mandate and 15/15 field geometry."
    )
    st.markdown(
        "<div class='rhgi-prism-narrative-frame'>"
        "CATEGORY 1 NARRATIVE: Harvest Trendline frames voter-economy pressure (inflation vs growth) "
        "for RHGI corridor planning toward the 20.7 million votes mandate."
        "</div>",
        unsafe_allow_html=True,
    )
    ...
    fig_harvest.update_layout(
        ...
        legend=dict(
            title=dict(
                text="Grassroots economic context",
                font=dict(family="Goldman, sans-serif", color=GOLD, size=13),
            ),
